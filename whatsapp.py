...
class WhatsappFilrt:
    ...
    def PreProcessing(self,filename):
        ...
        if(len(df.columns)==2):
            ...
            df["Time"] = Message[0].str.strip()
            
            Message1 = Message[1].str.split(":", n=1, expand=True)
            
            df["Name"] = Message1[0].str.strip()
            df["Text"] = Message1[1].str.strip()
            ...
            if Message1.shape[1] > 1:
                df["Text"] = Message1[1].str.strip()
            else:
                df["Text"] = ""
            
            df = df.dropna(subset=["Date", "Time", "Name"])
    ...
    def give_emoji_free_text(self,text):
        ...
        clean_text=" ".join(
            ...
            if not any(e in word for e in emoji_list)
        )
    ...
    def Wholeprocess(self,dataset):
        ...
        lambda x: self.give_emoji_free_text(
            str(x).encode("utf8")))
        dataset['Name']=dataset['Name'].apply(lambda x : self.deEmojify(str(x).lower()))
        ...
        unique_Frequency_Talker["Usage of word"] = (
            unique_Frequency_Talker["Repeated_count"] /
            Talker_chat["TW"].sum()
        ) * 100
        
        unique_Frequency_Lesser["Usage of word"] = (
            unique_Frequency_Lesser["Repeated_count"] /
            Less_chat["TW"].sum()
        ) * 100
                
        
           
        # Rename the count column safely
        ...
        Talker_Filter_list.columns=[
            'Repeated_count',
            'Frequency_Value'
        ]
        
        Less_Filter_list.columns=[
            'Repeated_count',
            'Frequency_Value'
        ]
        ...
        try:
            ...
            Talker_Filter_list=pd.concat(self.TalkerChecking(unique_Frequency_Talker,flirt_words,Talker_Filter_list))
        ...
        try:
            ...
            Talker_out=round(Talker_Filter_list['Flirt_Frequency'].sum()/Talker_Filter_list.shape["Repeated_count"],2)
        ...
        try:
             Less_Filter_list= pd.concat(self.LesserChecking(unique_Frequency_Lesser,flirt_words,Less_Filter_list))
        ...
        try:
            ...
            Less_out=round(Less_Filter_list['Flirt_Frequency'].sum()/Less_Filter_list.shape["Repeated_count"],2)
    ...
    def statsWhatsApp(self,dataset):
        ...
        day=datetime.date(l[2],l["Repeated_count"],l[1]).weekday()
